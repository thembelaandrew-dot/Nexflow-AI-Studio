...
interface ServiceItem {
  ...
  desc: string;
  label: string;
  color: 'brand-electricBlue' | 'brand-cyanAccent';
}

const TiltCard = ({ service }: { service: ServiceItem; idx: number }) => {
  ...
  const isCyan = service.color === 'brand-cyanAccent';
  ...
  return (
    ...
    >
      <div 
        className="glassmorphism p-8 rounded-2xl h-full flex flex-col justify-between group border border-white/5 hover:border-brand-electricBlue/40 relative overflow-hidden backdrop-blur-md bg-slate-900/40 shadow-lg transition-colors duration-300"
        style={canTilt ? { transform: "translateZ(15px)" } : {}}
      >
        ...
        <div className="relative z-10">
          <div className={`w-12 h-12 rounded-xl bg-slate-800/80 flex items-center justify-center ${isCyan ? 'text-brand-cyanAccent' : 'text-brand-electricBlue'} border border-slate-700/50 mb-6 group-hover:scale-110 group-hover:border-brand-cyanAccent/40 transition-transform duration-300 shadow-sm`}>
        ...
        <div className="mt-8 pt-4 border-t border-slate-800/50 flex items-center justify-between relative z-10">
          <span className={`text-[10px] ${isCyan ? 'text-brand-cyanAccent' : 'text-brand-electricBlue'} uppercase tracking-widest font-bold`}>{service.label}</span>
